...
-- Aviary Manager Excel import v5
-- Generated from My Birds_21_06_2026 22_34.xlsx
-- v5 uses fully-qualified public._import_* staging tables so Supabase SQL Editor cannot lose the import context.
...
-- No explicit transaction: Supabase SQL Editor may execute statements separately.
...
alter table public.birds add column if not exists legacy_comments text;
alter table public.birds add column if not exists legacy_notes text;
...
alter table public.pairs add column if not exists nest_number text;
...
alter table public.pairs add column if not exists legacy_descendants text;
alter table public.pairs add column if not exists legacy_clutches int;
alter table public.pairs add column if not exists legacy_eggs int;
...
-- Full import body is in the downloadable generated file /mnt/data/aviary_excel_import_v5.sql.
-- This repository file intentionally stores the compatibility header only.
